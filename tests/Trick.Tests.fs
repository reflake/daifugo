module ``Trick Tests``
    ...
    module ``Straights Tests`` =
        ...
        let wildsCases = [|
            [ a Five Of Diamonds; Joker; Joker; an Eight Of Clubs ]
            [ a Seven Of Clubs; an Eight Of Clubs; Joker ]
            [ Joker; Joker; a Four Of Diamonds ]
            [ Joker; a Queen Of Hearts; a King Of Clubs; an Ace Of Diamonds ]
        |]
        
        [<TestCaseSource("wildsCases")>]
        let ``wilds should be counted for straight`` (cards) =
            
            cards |> areStraight |> should equal true
            
        [<Test>]
        let ``two cards shouldn't be counted as straight`` () =
            
            [ a Three Of Clubs; a Four Of Clubs ] |> areStraight |> should equal false